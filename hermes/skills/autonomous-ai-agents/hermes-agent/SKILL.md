...
Check logs first:
```bash
grep -i "failed to send\\|error\\|not installed\\|No adapter" ~/.hermes/logs/gateway.log | tail -20
```
...
- **Gateway adapter not loading / missing dependency**: The Hermes-installed venv at `~/.hermes/hermes-agent/venv/` ships **without pip** (stripped for install size). Installing a package globally or via `pip install` in the terminal won't make it available to the gateway process. The correct command is:
  ```bash
  uv pip install <package> --python ~/.hermes/hermes-agent/venv/bin/python
  ```
  The `uvx --python ... pip install` variant does NOT work — `uvx` installs into a temporary ephemeral environment, not the target venv. Always use `uv pip install --python` for Hermes' venv.

### Platform-specific issues
- **Discord bot silent**: Must enable **Message Content Intent** in Bot → Privileged Gateway Intents.
- **Slack bot only works in DMs**: Must subscribe to `message.channels` event. Without it, the bot ignores public channels.
- **Telegram bot silent / "telegram bot has no access to messages" / "No adapter available for telegram"**: The gateway process runs from a venv that ships without pip. Even if `python-telegram-bot` is installed globally, the gateway won't see it. Fix:
  ```bash
  uv pip install python-telegram-bot --python ~/.hermes/hermes-agent/venv/bin/python
  hermes gateway restart
  ```
  Then check logs for `✓ telegram connected`. Also verify `TELEGRAM_BOT_TOKEN` is set in `~/.hermes/.env` and user ID is in `TELEGRAM_ALLOWED_USERS`.
