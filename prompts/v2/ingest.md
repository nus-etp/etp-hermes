...
This is the **v2 (experimental) arm of Layer 1**, running in parallel with the production pipeline for A/B comparison. Same candidate set, same mechanism, same dedup discipline — but the relevance judgment is yours to make from the goal, not from a rule list. Output paths are namespaced under `signals/v2/` so the two arms never touch each other's state.

Stay strictly within this arm: only write under `signals/v2/updates/` and append to `signals/v2/seen-urls.txt`. Do **not** touch `signals/updates/`, `signals/agent/`, `signals/seen-urls.txt`, `signals/briefs/`, or anything else under `signals/`.

## Task

`scripts/collect-candidates.py` already ran. It fetched every changed feed and curated source, parsed them, applied date windows, deduped against the production arm's seen list, and matched items to watchlist companies. Your job is the **relevance judgment**: filter the candidates against this arm's own seen list, judge each survivor, then write the kept items to `signals/v2/updates/<UTC-date>.md`.

Do **not** read `data/companies.json`, `data/feeds.json`, or either seen-urls file into context — everything you need is in `data/candidates.json`, and seen-URL membership checks are done with `grep`.
  ...
  "candidates": [
    {
      ...
      "link": "...",
      "dedup_key": "...",              // what gets appended to seen-urls
...
4. **Write outputs.** Let `K[]` be the candidates that passed the relevance pass.
   - Append the `dedup_key` of **every** candidate you judged — kept *and* dropped — to `signals/v2/seen-urls.txt`, one per line, deduplicated. Append only — do not rewrite. (Dropped keys are recorded so we don't re-judge the same junk tomorrow.)
   - If `K[]` is empty: write nothing else. Final stdout: `no new items`.
   - Otherwise write `signals/v2/updates/<YYYY-MM-DD>.md` (UTC date) containing the kept items, grouped by company (use the canonical `company` value as the `## ` heading). Format per item:
     ```
...
- Do not modify any file outside `signals/v2/updates/` and `signals/v2/seen-urls.txt`. In particular, never write to the production arm's paths (`signals/updates/`, `signals/seen-urls.txt`).
...
- Step 2 is the only permitted fetching, and a single failure there must not fail the run.
